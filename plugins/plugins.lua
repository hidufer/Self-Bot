...
local function list_all_plugins(only_enabled)
  ...
  local text = text..'\n\n'..nsum..' ÇİÒæäå åÇí äÕÈ ÔÏå\n\n'..nact..' ÇİÒæäå åÇí İÚÇá\n\n'..nsum-nact..' ÇİÒæäå åÇí ÛíÑİÚÇá'..tmp
  return text
end
...
local function list_plugins(only_enabled)
  ...
  local text = text..'\nÈÇÑÒÇÑí ãÌÏÏ ÇäÌÇã ÔÏ\n\n'..nact..' áÇíä åÇí İÚÇá:\n'..nsum..' áÇíä åÇí äÕÈ ÔÏå\n˜ÇäÇá ÓÇÎÊ ÑÈÇÊ ÖÏ ÇÓã\n@create_antispam_bot'
  return text
end
...
local function disable_plugin( name, chat )
  ...
  if not plugin_exists(name) then
    return ' '..name..' æÌæÏ äÏÇÑÏ'
  end
  local k = plugin_enabled(name)
  -- Check if plugin is enabled
  if not k then
    return ' '..name..' İÚÇá äíÓÊ'
  end
  -- Disable and reload
...
local function disable_plugin_on_chat(receiver, plugin)
  ...
  save_config()
  return ' '..plugin..' ÏÑ Çíä Ñæå ÛíÑİÚÇá ÇÓÊ'
end
...
local function reenable_plugin_on_chat(receiver, plugin)
  ...
  if not _config.disabled_plugin_on_chat[receiver][plugin] then
    return 'Çíä ÇİÒæäå ÛíÑİÚÇá äíÓÊ'
  end

  _config.disabled_plugin_on_chat[receiver][plugin] = false
  save_config()
  return ' '..plugin..' ãÌÏÏ İÚÇá ÔÏ'
end
...
local function run(msg, matches)
  ...
  if matches[1] == '-' and is_sudo(msg) then --after changed to moderator mode, set only sudo
    if matches[2] == 'plugins' then
    	return 'Çíä ÇİÒæäå äãíÊæÇäÏ ÛíÑİÚÇá ÔæÏ'
    end
